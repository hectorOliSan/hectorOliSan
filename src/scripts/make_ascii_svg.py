...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "assets", "avi-ascii.svg")
...
CONTRAST = 0.9
BRIGHTNESS = 1.0
GAMMA = 1.6           # >1 brightens mids -> face lands in sparser chars
SHARPEN = False
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">hector@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'hector@github:~$ whoami <tspan fill="{INK}">Héctor</tspan></text>')
